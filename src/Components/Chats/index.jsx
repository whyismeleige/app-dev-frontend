...
const MemberWidget = ({ member, showBelow = true, index }) => {
  const [isHovered, setIsHovered] = useState(false);
  const [userData, setUserData] = useState({});
  const memberRef = useRef(null);
  const [showAbove, setShowAbove] = useState(!showBelow);

  const fetchData = async () => {
    const data = await getDetails({ userId: member.userId });
    setUserData(data);
  };
  ...
  return (
    ...
    >
      {isHovered && (
        <div
          className={clsx(
            styles.memberWidgetContainer,
            showAbove ? styles.tooltipAbove : ""
          )}
        >
